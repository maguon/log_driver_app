...
import * as massLossListAction from './MassLossListAction'
import * as carInfoForDemageAction from '../../../actions/CarInfoForDemageAction'
import * as recordForDemageAction from '../../../actions/RecordForDemageAction'
import * as demageOpResultAction from '../../../actions/DemageOpResultAction'
import * as imageListForDemageAction from '../../components/demageInfo/ImageListForDemageAction'
...
const MassLossList = props => {
        console.log("props",props)
    const { massLossListReducer: { data: { massLossList, isComplete }, getMassLossList },
        massLossListReducer,
        getMassLossListMore,
        getCarInfo,
        getCarInfoWaiting,
    ...
    if (getMassLossList.isResultStatus == 1) {
        return (
            <Container>
        ...
        return (
            <Container style={globalStyles.listBackgroundColor}>
                <FlatList
                    ...
                    onEndReached={() => {
                        if (getMassLossList.isResultStatus == 2 && !isComplete) {
                            getMassLossListMore()
                        }
                    }}
                    ListFooterComponent={massLossListReducer.getMassLossListMore.isResultStatus == 1 ? ListFooterComponent : <View style={{ height: 10 }} />}
...
const mapStateToProps = (state) => {
    return {
        massLossListReducer: state.massLossListReducer
    }
}

const mapDispatchToProps = (dispatch) => ({
    getMassLossListMore: () => {
        dispatch(massLossListAction.getMassLossListMore())
    },
    getCarInfo: (param) => {
...
export default connect(mapStateToProps, mapDispatchToProps)(MassLossList)
