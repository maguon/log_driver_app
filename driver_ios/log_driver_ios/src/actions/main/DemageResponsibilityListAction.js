...
export const getDemageResponsibilityList = () => async (dispatch, getState) => {
    ...
    try {
        ...
        if (res.success) {
            ...
            dispatch({ type: actionTypes.demageResponsibilityListType.get_DemageResponsibilityList_failed, payload: { failedMsg: res.msg } })
        }
    } catch (err) {
        dispatch({ type: actionTypes.demageResponsibilityListType.get_DemageResponsibilityList_error, payload: { errorMsg: err } })
    }
}
...
export const getDemageResponsibilityListMore = () => async (dispatch, getState) => {
    ...
    if (demageResponsibilityListReducer.getDemageResponsibilityList.isResultStatus == 1) {
        ...
        if (!isComplete) {
            dispatch({ type: actionTypes.demageResponsibilityListType.get_DemageResponsibilityListMore_waiting, payload: {} })
            try {
                const url = `${base_host}/damage?${ObjectToUrl({
                ...
                if (res.success) {
                    if (res.result.length % pageSize != 0 || res.result.length == 0) {
                        dispatch({ type: actionTypes.demageResponsibilityListType.get_DemageResponsibilityListMore_success, payload: { demageResponsibilityList: res.result, isComplete: true } })
                    } else {
                        dispatch({ type: actionTypes.demageResponsibilityListType.get_DemageResponsibilityListMore_success, payload: { demageResponsibilityList: res.result, isComplete: false } })
                    }
                } else {
                    dispatch({ type: actionTypes.demageResponsibilityListType.get_DemageResponsibilityListMore_failed, payload: { failedMsg: res.msg } })
                }
            } catch (err) {
                dispatch({ type: actionTypes.demageResponsibilityListType.get_DemageResponsibilityListMore_error, payload: { errorMsg: err } })
            }
        } else {
