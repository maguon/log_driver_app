    ...
    return (
        <Card>
            <CardItem header bordered style={{justifyContent: 'space-between'}}>
                <Text
                    style={[globalStyles.midText, globalStyles.styleColor]}>
                    {insuranceItem.insure_type ? insuranceTypeList.find((item) => item.id == insuranceItem.insure_type).insuranceType : ''}</Text>
            ...
            <CardItem bordered>
                ...
                <Text style={globalStyles.smallText}><Text
                    style={{fontWeight: 'bold'}}>生效期：</Text>{insuranceItem.start_date ? moment(insuranceItem.start_date).format('YYYY-MM-DD') : ''} ～ {insuranceItem.end_date ? moment(insuranceItem.end_date).format('YYYY-MM-DD') : ''}
                </Text>
                </Body>
            </CardItem>
        </Card>
