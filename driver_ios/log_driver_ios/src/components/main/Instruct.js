                    ...
                    <View style={{flexDirection: 'row'}}>
                        <View style={{flexDirection: 'row', alignItems: 'center'}}>
                            <Text style={[globalStyles.midText, {color: '#8b959b', fontWeight: 'bold'}]}>
                                {item.addr_name ? item.addr_name : ''}{item.load_task_type == 2 &&
                    ...
                    <View style={{flexDirection: 'row', paddingTop: 10}}>
                        <View style={{flexDirection: 'row', flex: 1}}>
                            <Text style={{
                                ...
                                textAlign: 'left',
                                flex: 1
                            }}>计划运送：{item.plan_count ? item.plan_count : '0'}</Text>
                        </View>
                        <View style={{flexDirection: 'row', flex: 1}}>
                            <Text style={{fontSize: 11, color: '#8b959b', textAlign: 'right', flex: 1}}>实际运送：<Text
                            ...
                            <View style={{
                                ...
                                <View style={{flexDirection: 'row', alignItems: 'center', paddingLeft: 10}}>
                                    <Text style={{
                                        ...
                                        color: '#8b959b',
                                        fontWeight: 'bold'
                                    }}>{routeInfo.city_route_start ? routeInfo.city_route_start : ''}</Text>
                                    <MaterialCommunityIcons name='ray-start-arrow' size={20}
                                    ...
                                    <Text style={{
                                        ...
                                        paddingLeft: 5,
                                        fontWeight: 'bold'
                                    }}>{routeInfo.city_route_end ? routeInfo.city_route_end : ''}</Text>
                                </View>
                                <View style={{flexDirection: 'row', alignItems: 'center', paddingLeft: 40}}>
                                    <Text style={{fontSize: 15, color: '#8b959b', fontWeight: 'bold'}}><Text
                                        style={{color: '#d69aa5'}}>{routeInfo.distance ? `${routeInfo.distance}` : ''}</Text>公里</Text>
                                </View>
                            </View>
                        </View>
                        <View style={{flexDirection: 'row', paddingVertical: 10, justifyContent: 'space-between'}}>
                            <View style={{flexDirection: 'row'}}>
                                ...
                                <Text style={{
                                    ...
                                    paddingLeft: 5,
                                    color: '#8b959b'
                                }}>指定执行时间：{routeInfo.task_start_date ? moment(new Date(routeInfo.task_start_date)).format('YYYY-MM-DD HH:mm:ss') : ''}</Text>
                            </View>
                        </View>
                        <View style={{flexDirection: 'row', paddingBottom: 10, justifyContent: 'space-between'}}>
                            ...
                            <View style={{flexDirection: 'row'}}>
                                ...
                                <Text style={{
                                    ...
                                    paddingLeft: 5,
                                    color: '#8b959b'
                                }}>指定调度：{routeInfo.route_op_name ? routeInfo.route_op_name : ''}</Text>
                            </View>
                        ...
                        <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
                            <View style={{flexDirection: 'row'}}>
                                ...
                                <Text style={{
                                    ...
                                    paddingLeft: 5,
                                    color: '#8b959b'
                                }}>计划运送：{`${taskList.reduce((sum, value) => sum + (value.plan_count ? value.plan_count : 0), 0)}`}</Text>
                            </View>
                            <View style={{flexDirection: 'row'}}>
                                <Text style={{fontSize: 11, color: '#8b959b'}}>实际运送：<Text
                                    style={{color: styleColor}}>{routeInfo.car_count ? `${routeInfo.car_count}` : '0'}</Text></Text>
                            </View>
                            <View style={{flexDirection: 'row'}}>
                                <Text style={{
                                    fontSize: 11,
                                    color: styleColor
                                }}>{routeInfo.task_status == 9 ? '完成' : ''}</Text>
                            </View>
                        </View>
                    </View>
                    <FlatList
...
const mapDispatchToProps = (dispatch) => ({
    getRouteTaskList: (param) => {
        dispatch(actions.instructAction.getRouteTaskList(param))
    },
    setGetRouteTaskListWaiting: () => {
        dispatch(actions.instructAction.setGetRouteTaskListWaiting())
    }
})
