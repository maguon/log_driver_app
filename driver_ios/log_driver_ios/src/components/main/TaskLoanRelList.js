...
const renderItem = props => {
    ...
    return (
        <View style={styles.itemContainer}>
            ...
            <View style={styles.item}>
                <Text
                    style={[globalStyles.midText, globalStyles.styleColor, {fontWeight: 'bold'}]}>{city_route_start ? `${city_route_start}` : ''} - {city_route_end ? `${city_route_end}` : ''}</Text>
                <Text style={globalStyles.midText}><Text
                    style={globalStyles.styleColor}>{distance ? `${distance}` : ''}</Text>公里</Text>
            </View>
            <View style={styles.item}>
                <Text style={globalStyles.smallText}>装车数：{plan_count ? `${plan_count}` : ''}</Text>
                <Text
                    style={globalStyles.smallText}>计划执行时间：{task_plan_date ? `${moment(task_plan_date).format('YYYY-MM-DD')}` : ''}</Text>
            </View>
        </View>
    )
}
